class LBlockLeft extends BasicBlock {
  constructor(vectorPos) {
    super(vectorPos);
    this.blockColor = color('#90D753');
    this.blocks = [
      new BasicBlock(new p5.Vector(vectorPos.x, vectorPos.y), this.blockColor),
      new BasicBlock(
        new p5.Vector(vectorPos.x + BASIC_BLOCK_SIZE, vectorPos.y),
        this.blockColor
      ),
      new BasicBlock(
        new p5.Vector(vectorPos.x + 2 * BASIC_BLOCK_SIZE, vectorPos.y),
        this.blockColor
      ),
      new BasicBlock(
        new p5.Vector(vectorPos.x + 2 * BASIC_BLOCK_SIZE, vectorPos.y),
      ...
      new BasicBlock(
        new p5.Vector(vectorPos.x, vectorPos.y - BASIC_BLOCK_SIZE),
        this.blockColor
      ),
    ];
